...
function searchShows() {
	...
	case $LINES in
		0) 	echo No matching results found
...
function downloadShowList() {
	...
	curl -q "http://tviview.abc.net.au/iview/api2/?keyword=0-9" > "$TEMPDIR"/index.gz
	gunzip "$TEMPDIR"/index.gz
	mv "$TEMPDIR"/index.gz "$TEMPDIR"/index
	cat "$TEMPDIR"/index > "$TEMPDIR"/showindex
	rm "$TEMPDIR"/index

	curl -q "http://tviview.abc.net.au/iview/api2/?keyword=a-c" >> "$TEMPDIR"/index.gz
	gunzip "$TEMPDIR"/index.gz
	mv "$TEMPDIR"/index.gz "$TEMPDIR"/index
	cat "$TEMPDIR"/index >> "$TEMPDIR"/showindex
	rm "$TEMPDIR"/index

	curl -q "http://tviview.abc.net.au/iview/api2/?keyword=d-k" >> "$TEMPDIR"/index.gz
	gunzip "$TEMPDIR"/index.gz
	mv "$TEMPDIR"/index.gz "$TEMPDIR"/index
	cat "$TEMPDIR"/index >> "$TEMPDIR"/showindex
	rm "$TEMPDIR"/index

	curl -q "http://tviview.abc.net.au/iview/api2/?keyword=l-p" >> "$TEMPDIR"/index.gz
	gunzip "$TEMPDIR"/index.gz
	mv "$TEMPDIR"/index.gz "$TEMPDIR"/index
	cat "$TEMPDIR"/index >> "$TEMPDIR"/showindex
	rm "$TEMPDIR"/index

	curl -q "http://tviview.abc.net.au/iview/api2/?keyword=q-z" >> "$TEMPDIR"/index.gz
	gunzip "$TEMPDIR"/index.gz
	mv "$TEMPDIR"/index.gz "$TEMPDIR"/index
	cat "$TEMPDIR"/index >> "$TEMPDIR"/showindex
	rm "$TEMPDIR"/index
...
function getShow() {
	...
	case $SELECTION in
		a|A)	downloadShowList;;
		b|B);;
		c|C)	echo Enter showname \(What you want the file to be saved as\) :
			read SHOWNAME
...
function downloadShow() {
	...
	while [ $MAXRETRIES -gt $RETRY ]; do
		...
		case $SHOWEXT in
			"mp4")
				case $HOST in
					"Akamai")	SERVERPATH=$SHOWPATH.mp4;;
...
function convertFiles() {
	...
	while [ $COUNTER -lt $LINES ]; do
		...
		"$CURDIR"/ffmpeg -i "$CURDIR"/"$FILENAME.$SHOWEXT" -aspect 16:9 -acodec copy -vcodec copy "$CURDIR"/FIXED_"$FILENAME".avi
	done
	read -p "Press enter to continue..."
}
...
until [ 1 -lt 0 ]; do
	...
	case $OPTION in
		...
		2)	echo 
			...
			read -p "Press enter to continue..."
			;;
		3)	rm "$TEMPDIR"/download_list.txt
			rm "$TEMPDIR"/CompleteDownloads.txt
			rm "$TEMPDIR"/IncompleteDownloads.txt
			;;
		4)	runDownloadShow;;
